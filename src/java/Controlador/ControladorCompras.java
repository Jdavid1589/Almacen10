...
public class ControladorCompras extends HttpServlet {
    ...
    private void registrarCompra2(HttpServletRequest request, HttpServletResponse response)
        ...
        // Obtener el proveedor y el carrito de la sesión
        Proveedores proveedor = (Proveedores) request.getSession().getAttribute("proveedorId");
        List<Compras> carrito = (List<Compras>) request.getSession().getAttribute("carrito");

        if (proveedor == null || carrito == null || carrito.isEmpty()) {
            // Error, no se puede proceder con la compra
            request.setAttribute("mensajeError", "Debe seleccionar un proveedor y agregar productos.");
            request.getRequestDispatcher("Vistas/Lista_Compras_Articulos.jsp").forward(request, response);
            return;
        }

        // Iterar sobre cada compra en el carrito
        for (Compras compra : carrito) {
            // Completar los detalles de la compra
            compra.setProveedorId(proveedor.getIdProveedor());
            compra.setFecha(request.getParameter("fecha"));
            compra.setTotalCompra(compra.getArticulos().stream()
                    .map(item -> item.getCostoArticulo().multiply(item.getCantidad()))
        ...
        request.getSession().removeAttribute("proveedorId");
    ...
    private void registrarCompra(HttpServletRequest request, HttpServletResponse response)
        ...
        if (fechastr == null || carrito == null || carrito.isEmpty()) {
            // Error, no se puede proceder con la compra
            request.setAttribute("mensajeError", "Valdar datos ingresado  Error! .");
            request.getRequestDispatcher("Vistas/Lista_Compras_Articulos.jsp").forward(request, response);
            return;
        }
